# -*- coding: utf-8 -*-
#@+leo-ver=cub-1-thin
#@0 [ekr.20081121110412.2] @f ./obsolete/tkGui.py
#@@first
...
class tkinterGui(leoGui.leoGui):
    ...
    #@> runOpenDirectoryDialog (tkGui)
    def runOpenDirectoryDialog(self, title, startdir):
        """Create and run a Tk open directory dialog ."""

        dirName = tkFileDialog.askdirectory(title=title, initialdir=startdir, mustexist="true")

        return dirName

    #@ runOpenFileDialog (tkGui)
    ...
    def set_focus(self, c, w):
        ...
        if w:
            try:
                ...
                if trace or g.unitTesting:
                    g.es_exception()
                return False

    #@< Font
    #@> tkGui.getFontFromParams
    ...
    def makeScriptButton(
        ...
        def executeScriptCallback(
            event=None, args=args, b=b, c=c, buttonText=buttonText, p=p and p.copy(), script=script
        ):
...
class leoTkinterComparePanel(leoCompare.leoCompare, leoTkinterDialog):
    ...
    #@+others
    #@> Birth...
    #@>  tkinterComparePanel.__init__
    def __init__(self, c):
        # Init the base class.
        leoCompare.leoCompare.__init__(self, c)
        leoTkinterDialog.__init__(self, c, "Compare files and directories", resizeable=False)
    ...
    def finishCreate(self):
        ...
        # File names.
        for i, option in ((0, "compare_file_1"), (1, "compare_file_2"), (2, "output_file")):
    ...
    def browser(self, n):
        ...
        fileName = tkFileDialog.askopenfilename(
            title="Choose compare file" + n, filetypes=types, defaultextension=".txt"
        )
...
class tkinterAboutLeo(leoTkinterDialog):
    ...
    #@ tkinterAboutLeo.onAboutLeoEmail
    ...
    #@ tkinterAboutLeo: setArrowCursor, setDefaultCursor
...
class tkinterAskOkCancelNumber(leoTkinterDialog):
    ...
    #@ tkinterAskOKCancelNumber.createFrame
    def createFrame(self, message):
    ...
    #@ tkinterAskOKCancelNumber.okButton, cancelButton
    def okButton(self):
    ...
    #@ tkinterAskOKCancelNumber.onKey
    def onKey(self, event):
...
class tkinterAskOkCancelString(leoTkinterDialog):
    ...
    #@ tkinterAskOkCancelString.createFrame
    def createFrame(self, message):
        """Create the frame for a number dialog."""
    ...
    #@ tkinterAskOkCancelString.okButton, cancelButton
    def okButton(self):
    ...
    #@ tkinterAskOkCancelString.onKey
    def onKey(self, event):
        ch = event.char.lower()
...
class tkFindTab(leoFind.findTab):
    ...
    def createFrame(self, parentFrame):
        ...
        def toFind(event, w=ftxt):
            return setFocus(w)

        def toChange(event, w=ctxt):
            return setFocus(w)
        ...
        def insertTab(w):
            ...
            if data:
                ...
                start = end = w.getInsertPoint()
            w.replace(start, end, "\t")
            return "break"

        def insertFindTab(event, w=ftxt):
            return insertTab(w)

        def insertChangeTab(event, w=ctxt):
            return insertTab(w)
        ...
        for i in range(numberOfColumns):
            for var, name, val in radioLists[i]:
                ...
                if val == None:
                    box.button.configure(state="disabled")
                box.bindHotKey(ftxt)
                box.bindHotKey(ctxt)
        ...
        if self.optionsOnly:
            ...
            for text, boxKind, frame, callback in (
                ...
                w = underlinedTkButton(boxKind, frame, text=text, command=callback)
                buttons.append(w)
                if text == defaultText:
    ...
    #@<2 Support for minibufferFind class (tkFindTab)
    #@> getOption
...
class leoTkinterBody(leoFrame.leoBody):
    ...
    def createBindings(self, w=None):
        ...
        def onFocusOut(event, c=c):
            ...
            self.setEditorColors(bg=c.k.unselected_body_bg_color, fg=c.k.unselected_body_fg_color)
        ...
        table2 = (('<Button-2>', frame.OnPaste, k.masterClickHandler),)
    ...
    def createTextWidget(self, parentFrame, p, name):
        ...
        bodyXBar = Tk.Scrollbar(parentFrame, name='bodyXBar', orient="horizontal")
    ...
    #@ Configuration (Tk spelling) (tkBody)
    ...
    #@ Menus (tkBody) (May cause problems)
...
class leoTkinterFrame(leoFrame.leoFrame):
    ...
    #@ createSplitterComponents (tkFrame)
    ...
    def enableTclTraces(self):
        ...
        if True:
            ...
            def focusIn(event):
                g.pr("Focus in  %s (%s)" % (event.widget, event.widget.winfo_class()))

            def focusOut(event):
                g.pr("Focus out %s (%s)" % (event.widget, event.widget.winfo_class()))
    ...
    #@< tkFrame.createLeoSplitters & helpers
    #
    ...
    #@<2 class tkStatusLineClass (tkFrame)
    ...
    def setMinibufferBindings(self):
        ...
        table2 = (('<Button-2>', k.masterClickHandler),)
    ...
    #@< Configuration (tkFrame)
    #@> configureBar (tkFrame)
    ...
    #@ configureBarsFromConfig (tkFrame)
    ...
    #@ setInitialWindowGeometry (tkFrame)
    ...
    #@ reconfigurePanes (use config bar_width) (tkFrame)
    ...
    def contractLogPane(self, event=None):
        ...
        f = self
        r = min(1.0, f.secondary_ratio + 0.1)  # 2010/02/23: was f.ratio
    ...
    def expandLogPane(self, event=None):
        '''Expand the log pane.'''
        f = self
        r = max(0.0, f.secondary_ratio - 0.1)  # 2010/02/23: was f.ratio
        f.divideLeoSplitter(not f.splitVerticalFlag, r)
...
class leoTkinterLog(leoFrame.leoLog):
    ...
    def createCanvasWidget(self, parentFrame):
        ...
        try:
            ...
            pass  # Could be a user error.

        return w

    #@ tkLog.createTextWidget
    ...
    def makeTabMenu(self, tabName=None, allowRename=True):
        ...
        if tabName:
            ...
            label = g.choose(tabName in ('Find', 'Spell'), 'Hide This Tab', 'Delete This Tab')
    ...
    #@> tkLog.configureBorder & configureFont
    ...
    def createTab(self, tabName, createText=True, wrap='none'):
        ...
        if createText:
            ...
            if wrap not in ('none', 'char', 'word'):
                wrap = 'none'
            try:
                w.configure(bg=bg, wrap=wrap)
            except Exception:
                pass  # Could be a user error.
    ...
    #@ newTabFromMenu & newCanvasTabFromMenu
...
class leoTkinterMenu(leoMenu.leoMenu):
    ...
    def insert_cascade(self, parent, index, label, menu, underline):
        ...
        if parent:
            return parent.insert_cascade(index=index, label=label, menu=menu, underline=underline)
    ...
    #@< Methods with other spellings (Tkmenu)
    #@> clearAccel
...
class leoTkinterTree(leoFrame.leoTree):
    ...
    def redraw_now(self, p=None, scroll=True, forceDraw=False):
        ...
        if trace:
            g.trace(self.redrawCount, g.callers(8))
    ...
    #@ redraw_after_head_changed (tkTree)
    ...
    def scrollTo(self, p=None):
        ...
        if not p or not c.positionExists(p):
            p = c.currentPosition()
            if trace:
                g.trace('*** current position', p, p.stack)
        if not p or not c.positionExists(p):
            if trace:
        ...
        try:
            if trace:
                g.trace('***', p, p.stack, 'exists', c.positionExists(p))
            h1 = self.yoffset(p)
    ...
    def yoffsetTree(self, p, p1, isTop):
        ...
        verbose = True
        if trace:
            g.trace('entry', 'root', p, p.stack, 'target', p1, p1.stack)
        ...
        for p2 in theIter:
            ...
            if p2 == p1:
                if trace and verbose:
                    g.trace('returns', h, p1.h)
                return h, True
            h += self.line_height
            if p2.isExpanded() and p2.hasChildren():
                child = p2.firstChild()
                if trace and verbose:
                    g.trace('recursive call')
        ...
        if trace:
            g.trace('not found', h, p1.h)
        return h, False

    #@< tree.edraw_after methods (new)
    ...
    def findEditWidget(self, p):
        ...
        if p and c:
            ...
            if aTuple:
                ...
                if trace:
                    g.trace('oops: not found', p, g.callers())
                return None
    ...
    def editLabel(self, p, selectAll=False, selection=None):
        ...
        if p and w:
            ...
            if trace:
                g.trace('*** Error: no edit widget for %s' % p)
...
#@<2 class leoTkTextWidget (Tk.Text)
class leoTkTextWidget(Tk.Text):
    ...
    #@ Index conversion (leoTextWidget)
    #@> w.toPythonIndexToRowCol
    # New in Leo 4.6 b1.
    ...
    def leoMoveCursorHelper(self, kind, extend=False, linesPerPage=15):
        ...
        d = {
            ...
            'right': g.choose(
                Tk.Text.compare(w, "%s.%s" % (i, j + 1), ">=", '%s.end' % (i)),
                '%s.%s' % (i + 1, 0),
                '%s.%s' % (i, j + 1),
            ),
            'start-line': '%s.0' % (i),
        ...
        newIns = d.get(kind)
        if trace:
            g.trace('**Tk**', kind, 'extend', extend, anchor, ins, newIns)
    ...
    def setAllText(self, s, new_p=None):  # tkTextWidget
        ...
        if s:
            Tk.Text.insert(w, '1.0', s)  # The 'if s:' is a workaround for a fedora bug.
