...
class plainTextWidget(baseTextWidget):
    ...
    def _setAllText(self, s):
        return self.widget.ChangeValue(s)
    ...
    def _setInsertPoint(self, i):
        return self.widget.SetInsertionPoint(i)
...
class stcWidget(baseTextWidget):
    ...
    def initStc(self):
        ...
        if wx.Platform == '__WXMSW__':
            ...
            w.StyleSetSpec(stc.STC_STYLE_DEFAULT, 'fore:#000000,back:#FFFFFF,face:Courier')
        else:
            w.StyleSetSpec(stc.STC_STYLE_DEFAULT, 'fore:#000000,back:#FFFFFF,face:Courier,size:9')
    ...
    def _get(self, i, j):
        return self.widget.GetTextRange(i, j)
    ...
    def _getInsertPoint(self):
        return self.widget.GetCurrentPos()

    def _getLastPosition(self):
        return self.widget.GetLength()
    ...
    def _scrollLines(self, n):
        return self.widget.ScrollToLine(n)

    def _see(self, i):
        g.trace('oops', i)  # Should not be called.

    def _setAllText(self, s):
        return self.widget.SetText(s)
    ...
    def _setSelectionRange(self, i, j):
        g.trace('oops', i, j)  # Should not be called.
...
class wxLeoBody(leoFrame.leoBody):
    ...
    def onBodyChanged(self, undoType, oldSel=None, oldText=None, oldYview=None):
        ...
        if not p:
            return g.trace('no p!')
        if self.frame.lockout > 0:
            return g.trace('lockout!', g.callers())
...
class wxLeoFrame(leoFrame.leoFrame):
    ...
    if wx.Platform == '__WXMSW__':
        ...
        def onActivate(self, event):
            if g.app.killed or self.killed:
                return
            if event.GetActive():
...
class wxLeoLog(leoFrame.leoLog):
    ...
    def createTab(self, tabName, createText=True, wrap='none'):  # wxLog.
        ...
        if createText:
            ...
            w = plainTextWidget(self.c, win, name='text tab:%s' % tabName)
...
class wxLeoMenu(leoMenu.leoMenu):
    ...
    def defineOpenWithMenuCallback(self, command):
        ...
        def wxOpenWithMenuCallback(event=None, command=command):
            try:
                self.c.openWith(data=command)
            except:
                g.pr(traceback.print_exc())
...
class wxLeoStatusLine:
    ...
    def update(self):
        ...
        if not self.isVisible or self.c.frame.killed:
            return
...
class wxLeoTree(baseNativeTree.baseNativeTreeWidget):
    ...
    def traceItem(self, item):
        ...
        if v:
            if s == v.h:
                return 'item %s: %s' % (id(item), s)
            else:
                return '*** item %s: %s, mismatched vnode: %s %s' % (id(item), s, id(node), v.h)
        else:
    ...
    def childIndexOfItem(self, item):
        ...
        if not item or not item.IsOk():
            if trace:
                g.trace('invalid item', self.traceItem(item))
            return 0
        ...
        while item2 and item2.IsOk():
            ...
            if item2 == item:
                if trace:
                    g.trace('childIndex is', n, self.traceItem(item))
                return n
            else:
                n += 1
        ...
        if keyname is None:
            if 0 < keycode < 27:
                ...
                if isStc:
                    ...
                    if shift:
                        keyname = self.shift(keycode, uchar)
                    else:
                        keyname = self.unshift(keycode, uchar)
                else:
                    keyname = uchar
            else:
                # No unicode support.
