...
def _descendHdl(cmdrUnl, unlList):
    ...
    for idx1, hdl in enumerate(unlList):
        if idx1 == lastUnlIdx:
            ...
            soFarList = [
                appendList(stk, (childVnode, idx2))
...
class MenuPopUp(QtWidgets.QMenu):
    ...
    def __init__(self, babelG, parent=None):
        ...
        actTDL.triggered.connect(lambda x: self._info('The menu title is "Leo-Babel Menu"'))
        for eidx, labelActionHint in enumerate(
            (
                LabelActionHint(
                    '&Copy UNL to clipboard',
...
def babelExec(event):
    ...
    def _babelExec(babelG, babelCmdr, babelRoot):
        ...
        exec(
            code, gld
        )  # This adds all the variables set by "code" to dictionary gld. [It also adds Python built-in's to gld.]

        # Copy Babel Parameter settings from gld to babelCmdr
        for paramName in [
            'babel_color_information',
            'babel_color_stderr',
            'babel_color_stdout',
            'babel_interpreter_python',
            'babel_interpreter_shell',
            'babel_node_creation',
            'babel_polling_delay',
        ...
        leoG.es(
            f'{babelCmdr.babel_prefix_information}Script\'s CWD: "{cwd}"',
            color=babelCmdr.babel_color_information,
            tabName='Babel' if babelCmdr.babel_tab_babel else 'Log',
        )
        leoG.es(
            f'{babelCmdr.babel_prefix_information}Command list: {cmdList}',
            color=babelCmdr.babel_color_information,
            tabName='Babel' if babelCmdr.babel_tab_babel else 'Log',
        )
        ...
        def composeScript(c, p, s, forcePythonSentinels=True, sentinels=True):
            ...
            if s.strip():
                ...
                script = c.atFileCommands.stringToString(
                    p.copy(), s, forcePythonSentinels=forcePythonSentinels, sentinels=sentinels
                )
    ...
    def itp(
        ...
        if babelCmdr.cmdDoneFlag:
            if babelCmdr.cmdDoneStdPolled and babelCmdr.cmdDoneErrPolled:
                ...
                leoG.es(
                    babelCmdr.babel_prefix_information + babelCmdr.termMsg,
                    color=babel_color_information,
                    tabName='Babel' if babelCmdr.babel_tab_babel else 'Log',
                )
                leoG.es(
                    babelCmdr.babel_prefix_information + babelCmdr.etMsg,
                    color=babel_color_information,
                    tabName='Babel' if babelCmdr.babel_tab_babel else 'Log',
                )
    ...
    def makeBabelNodes(cmdrRes, resultsRoot, reo, ree, termMsg, etMsg):
        ...
        undoer = cmdrRes.undoer
        undoType = 'Leo-Babel Add Nodes'
    ...
    def scrOrResRoot(leoCmdrB, babelCmdr, babelG, babelRoot, scrOrRes):
        ...
        if getattr(babelCmdr, scrOrRes, None) is None:
            ...
            if not posList:
                raise babelG.babel_api.BABEL_UNL_NO_POS(
                    f'{scrOrRes.capitalize()} Root UNL does not match any position.'
                )
            else:
                posX = posList[0]
                if len(posList) > 1:
