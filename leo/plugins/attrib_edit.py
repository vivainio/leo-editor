#@+leo-ver=cub-1-thin
#@0 [tbrown.20091029123555.5319] @f ../plugins/attrib_edit.py
#@+<< docstring >>
#@> << docstring >>
...
class AttributeGetterUA(AttributeGetter):
    ...
    def createAttrib(self, v, gui_parent=None):
        path, ok = QtWidgets.QInputDialog.getText(
            gui_parent, "Enter attribute path", "Enter path to attribute (space separated words)"
        )
...
class attrib_edit_Controller:
    ...
    #@+others
    #@> __init__ & reloadSettings (attrib_edit_Controller)
    def __init__(self, c):
        self.c = c
        c.attribEditor = self
...
#@< cmd_Modes (attrib_edit_Controller)
...
#@ cmd_Manage (attrib_edit_Controller)
...
#@ cmd_Create (attrib_edit_Controller)
...
#@ cmd_CreateReadonly (attrib_edit_Controller)
def Xcmd_CreateReadonly(c):
    c.attribEditor.createAttrib(readonly=True)


#@ cmd_Scan (attrib_edit_Controller)
