...
def dump_handlers(
    ...
    if dump_handlers:
        ...
        aList = [method_name(obj.handlers.get(z)) for z in obj.handlers]
        ...
        if native:
            ...
            for i in range(3):
                ...
                for j in range(2):
                    ...
                    for k in range(4):
                        grand_child = child.new_child(content='grand-child %s.%s.%s' % (i, j, k))
...
class CoreFrame(leoFrame.LeoFrame):
    ...
    def getShortCut(self, *args: Args, **kwargs: KWargs) -> None:
        return None
...
class TopFrame:
    ...
    def finishCreateLogPane(self, *args: Args, **kwargs: KWargs) -> None:
        pass
...
class LeoLog(npyscreen.MultiLineEditable):
    ...
    def h_cursor_line_down(self, ch_i: int) -> None:
        ...
        if trace:
            g.trace('n: %s, start: %s, line: %s' % (n, self.start_display_at, self.cursor_line))
...
class LeoMLTree(npyscreen.MLTree):
    ...
    def _redraw(self, clear: bool) -> None:
        ...
        if clear is True or clear is None and self._last_start_display_at != self.start_display_at:
            self.clear()
...
class LeoValues(npyscreen.TreeData):
    ...
    def refresh_cache(self) -> int:
        ...
        self.last_len = n
        if trace:
            g.trace('%3s vis: %3s generation: %s' % (self.n_refreshes, n, c.frame.tree.generation))
        return n
...
class StatusLineWrapper(StringTextWrapper):
    ...
    def disable(self, *args: Args, **kwargs: KWargs) -> None:
        pass

    def enable(self, *args: Args, **kwargs: KWargs) -> None:
        pass
    ...
    def update(self, *args: Args, **kwargs: KWargs) -> None:
        self.widget.update()
