...
class PluginCatalog:
    ...
    def make_document(self, doc_strings):
        ...
        big_doc += nodes.title(text="Plugins listing generated %s" % time.asctime())
        ...
        if not opt.no_summary:
            ...
            big_doc += nodes.section('', nodes.title(text="Plugins summary"), alpha_list, def_list)
        ...
        for doc in doc_strings:
            ...
            if not opt.no_summary:
                firstpara = self.first_text(doc[2]) or nodes.paragraph(text='No summary found')
                reference = nodes.reference('', refid=section['ids'][0], name=doc[0], anonymous=1)
                reference += nodes.Text(doc[0])
                def_list += nodes.definition_list_item(
                    '', nodes.term('', '', reference), nodes.definition('', firstpara)
                )
    ...
    def make_parser():
        ...
        parser = optparse.OptionParser(
            "Usage: plug_catalog.py [options] dir1 [dir2 ...] output.html"
        )
        ...
        parser.add_option(
            ...
            action="append",
            type="string",
            help="Add a location to the list to search",
            default=[],
        )
        parser.add_option("--css-file", type="string", help="Use this CSS file in the HTML output")
        parser.add_option(
            "--max-files", type="int", help="Stop after this many files, mainly for testing"
        )
        parser.add_option(
            "--include-contents",
            action="store_true",
        ...
        parser.add_option(
            "--no-summary", action="store_true", default=False, help="Don't generate the summary"
        )
        parser.add_option(
            "--show-paths",
            action="store_true",
        ...
        parser.add_option(
            "--output", type="string", default=None, help="Filename for the html output"
        )
        parser.add_option(
            "--xml-output",
            type="string",
    ...
    def run(self):
        ...
        open(opt.output, 'wb').write(
            publish_from_doctree(big_doc, writer_name='html', settings_overrides=settings_overrides)
        )
        err("Wrote '%s'\n" % opt.output)
        ...
        if opt.xml_output:
            open(opt.xml_output, 'wb').write(
                publish_from_doctree(
                    big_doc, writer_name='xml', settings_overrides={'indents': True}
                )
            )
            err("Wrote '%s'\n" % opt.xml_output)
