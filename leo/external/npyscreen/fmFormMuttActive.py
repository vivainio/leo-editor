...
class ActionControllerSimple:
    ...
    def add_action(self, ident, function, live):
        ...
        self._action_list.append({'identifier': ident, 'function': function, 'live': live})
...
class TextCommandBoxTraditional(TextCommandBox):
    ...
    def handle_input(self, inputch):
        ...
        if (
            (inputch in self.always_pass_to_linked_widget)
            or (inputchstr in self.always_pass_to_linked_widget)
            or (input_unctrl in self.always_pass_to_linked_widget)
        ):
