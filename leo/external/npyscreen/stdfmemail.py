...
class EmailTree(npyscreen.MultiLineTreeNew):
    ...
    def h_select(self, ch):
        ...
        try:
            value = [
                weakref.proxy(self.values[self.cursor_line]),
            ]
        except TypeError:
...
class EmailViewFm(npyscreen.SplitFormWithMenus):
    ...
    SHORT_HEADER_LIST = ('from', 'to', 'cc', 'bccdate', 'subject', 'reply-to')
    ...
    def create(self):
        ...
        self.wDate = self.add(npyscreen.TitleText, begin_entry_at=10, editable=False, name="Date:")
    ...
    def _savePartToFile(self, messagePart):
        ...
        if not fn:
            ext = mimetypes.guess_extension(
                messagePart.get_content_type()
            )  # Bug in python returns .ksh for text/plain.  Wait for python fix?
            if not ext:
                # generic extension?
                ext = '.bin'
            fn = 'emailpart%s' % (ext)
