...
class _FormBase(
    ...
    def draw_title_and_help(self):
        ...
        if self.help and self.editing:
            try:
                ...
                self.add_line(
                    0,
                    self.curses_pad.getmaxyx()[1] - len(help_advert) - 2,
                    help_advert,
    ...
    def add_widget(
        ...
        _w = widgetClass(self, rely=rely, relx=relx, max_height=max_height, *args, **keywords)
...
class Form(
    form_edit_loop.FormDefaultEditLoop,
    _FormBase,
):
