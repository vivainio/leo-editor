...
class MultiLine(widget.Widget):
    ...
    def update(self, clear=True):
        ...
        try:
            no_change = (
                self._safe_to_display_cache
                and self._last_value is self.value
                and self.values == self._last_values
                and self.start_display_at == self._last_start_display_at
                and clear != True
                and self._last_cursor_line == self.cursor_line
                and self._last_filter == self._filter
                and self.editing
            )
        except Exception:
            no_change = False
        ...
        if trace:
            ...
            if isinstance(val, npysTree.TreeData):
                val = val.get_content()
            g.trace('changed: %5s, cursor_line: %s %s' % (not no_change, self.cursor_line, val))
    ...
    def _print_line(self, line, value_indexer):
        ...
        # Sets line.value
        if trace:
            g.trace(value_indexer, line.value.get_content())
    ...
    def _set_line_highlighting(self, line, value_indexer):
        ...
        if (value_indexer == self.value) and (self.value is not None):
    ...
    def move_next_filtered(self, include_this_line=False, *args):
        ...
        try:
            if (
                self.cursor_line - self.start_display_at > len(self._my_widgets)
                or self._my_widgets[self.cursor_line - self.start_display_at].task == MORE_LABEL
            ):
                if self.slow_scroll:
                    self.start_display_at += 1
                else:
    ...
    def set_up_handlers(self):
        ...
        if self.exit_left:
            self.handlers.update({curses.KEY_LEFT: self.h_exit_left})
        if self.exit_right:
            self.handlers.update({curses.KEY_RIGHT: self.h_exit_right})
    ...
    def edit(self):
        ...
        while self.editing:
            if trace:
                g.trace('(MultiLine:%s) LOOP' % self.__class__.__name__)
            ...
            ##  curses.flushinp()
        if trace:
            g.trace('(MultiLine:%s) DONE' % self.__class__.__name__)
    ...
    def h_find_shortcut_action(self, _input):
        ...
        for r in range(len(self.values)):
            if hasattr(self.values[r], self.shortcut_attribute_name):
                ...
                if (
                    getattr(self.values[r], self.shortcut_attribute_name) == _input
