...
class TitleText(widget.Widget):
    ...
    def make_contained_widgets(self):
        self.label_widget = textbox.Textfield(
            self.parent,
            relx=self.relx,
            rely=self.rely,
        ...
        if self.label_widget.on_last_line and self.use_two_lines:
            ...
            self.use_two_lines = False
            self.label_widget = textbox.Textfield(
                self.parent, relx=self.relx, rely=self.rely, width=ab_label + 1, value=self.name
            )
        ...
        self.entry_widget = self.__class__._entry_type(
            self.parent,
